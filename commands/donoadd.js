...
module.exports = {
    ...
    run: async(client, message, args) => {
      ...
      if(user === `${config.get(`owner`)}`) return message.reply(`${emoji.get(`errado`)} | Essa pessoa já é o dono do bot!`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
      ...
      message.reply(`${emoji.get(`certo`)} | Permissão de dono adicionada!`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
