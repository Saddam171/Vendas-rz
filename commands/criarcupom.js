...
module.exports = {
    ...
    run: async(client, message, args) => {
      ...
      if(!args[0]) return message.reply(`${emoji.get(`errado`)} | Você não deu nenhum ID a esse cupom!`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
      ...
      if(args[0] === `${db.get(`${args[0]}.idcupom`)}`) return message.reply(`${emoji.get(`errado`)} | Esse ID de cupom já é existente!`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));

      message.reply(`${emoji.get(`certo`)} | Criado com sucesso!`)
