...
module.exports = {
    ...
    run: async(client, message, args) => {
      if(message.author.id !== `${perms.get(`${message.author.id}_id`)}`) return message.reply(`${emoji.get(`certo`)} | Você não está na lista de pessoas!`)
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .addField(`${emoji.get(`outros`)} | Status hoje:`, `Vendas realizadas: ${hojepedidos || "0"} \nDinheiro arrecadado: R$ ${hojerecebimentos || "0"}`, false)
        .addField(`${emoji.get(`outros`)} | Status 7 dias:`, `Vendas realizadas: ${setedias || "0"} \nDinheiro arrecadado: R$ ${setediasrec || "0"}`, false)
        .addField(`${emoji.get(`outros`)} | Status 30 dias:`, `Vendas realizadas: ${setedias2 || "0"} \nDinheiro arrecadado: R$ ${setediasrec2 || "0"}`, false)
