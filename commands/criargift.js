    ...
    run: async(client, message, args) => {
      ...
      const row = new Discord.MessageActionRow()
        ...
        const embed = await message.reply({ embeds: [new Discord.MessageEmbed()
          .setTitle(`${config.get(`title`)} | Criação de Gift Card ${emoji.get(`presente`)}`)
          .addField(`${emoji.get(`caixa`)} Estoque:`, `Nenhum...`)
          .addField(`${emoji.get(`desc`)} Código:`, `${gerado}`)
        ...
        const interação = embed.createMessageComponentCollector({ componentType: "BUTTON", });
          interação.on("collect", async (interaction) => {
             ...
             collector.on("collect", message => {
               ...
               for (let i = 0; i != estoque.length; i++) {
                 ...
                 if(i + 1 === estoque.length) {
                   ...
                   msg.edit(`${emoji.get(`certo`)} | Gift \`${gerado}\`\ Criado com sucesso!`)
                   const embednew = new Discord.MessageEmbed()
                     .setTitle(`${config.get(`title`)} | Criação de Gift Card`)
                     .addField(`${emoji.get(`caixa`)} Estoque:`, `${texto}`)
                     .addField(`${emoji.get(`desc`)} Código:`, `${gerado}`)
           ...
           if (interaction.customId === "cancelar") {
             embed.delete()
             message.channel.send(`${emoji.get(`errado`)} | Cancelado...`)
           }
         })
       }
     };
